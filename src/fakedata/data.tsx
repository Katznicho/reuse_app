export const products= [
    {
      id: 1,
      name: 'HP Pro Lite',
      price: 100,
      description: '  Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempora labore nihil placeat libero pariatur accusamus incidunt perspiciatis reiciendis repellendus totam?',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:'https://images.unsplash.com/file-1636585210491-f28ca34ea8ecimage?dpr=2&auto=format&fit=crop&w=416&q=60',
      rating:5
    },
    {
      id: 2,
      name: 'Swed Shoes',
      price: 200,
      description: '  Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempora labore nihil placeat libero pariatur accusamus incidunt perspiciatis reiciendis repellendus totam?',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:'https://media.istockphoto.com/id/1425880214/photo/leather-mens-shoe-on-white-background.jpg?b=1&s=170667a&w=0&k=20&c=HsgDQAUh9FyNzEkJo4ZsfreVCVYpOdN5ax2oSZPqFV4=',
      rating:4
    
    },
    {
      id: 3,
      name: 'Canvas Shoes',
      price: 100,
      description: '  Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempora labore nihil placeat libero pariatur accusamus incidunt perspiciatis reiciendis repellendus totam?',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:
        'https://images.unsplash.com/photo-1542291026-7eec264c27ff?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8c2hvZXN8ZW58MHx8MHx8&auto=format&fit=crop&w=500&q=60',
      screen: 'FitnessTrainers',
      rating:2
    },
    {
      id: 4,
      name: 'Cupboard',
      price: 200,
      description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempora labore nihil placeat libero pariatur accusamus incidunt perspiciatis reiciendis repellendus totam?',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:
        'https://images.unsplash.com/photo-1632935187086-49a9d8027292?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8OXx8ZnVybml0dXJlc3xlbnwwfHwwfHw%3D&auto=format&fit=crop&w=500&q=60',
      screen: 'FitnessTrainers',
      rating:3
    },
  ];

  
  export const packages = [
    {
      id: 1,
      name: 'Lite',
      price: 100,
      description: 'Package for beginners',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:
        'https://media.istockphoto.com/id/1289416200/photo/athletic-woman-doing-the-full-body-cardio-workout.jpg?b=1&s=170667a&w=0&k=20&c=PgzzhwgGkvrqOTtTi79q3voWp83FOhhWE4XQCyJd7hQ=',
      screen: '',
    },
    {
      id: 2,
      name: 'Pro',
      price: 200,
      description: 'Best for professionals',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:
        'https://media.istockphoto.com/id/1289416200/photo/athletic-woman-doing-the-full-body-cardio-workout.jpg?b=1&s=170667a&w=0&k=20&c=PgzzhwgGkvrqOTtTi79q3voWp83FOhhWE4XQCyJd7hQ=',
      screen: '',
    },
    {
      id: 3,
      name: 'Premium',
      price: 100,
      description: 'For those who want to be the best',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:
        'https://images.unsplash.com/photo-1594381898411-846e7d193883?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTR8fGZpdG5lc3N8ZW58MHx8MHx8&auto=format&fit=crop&w=500&q=60',
      screen: '',
    },
    {
      id: 4,
      name: 'Ultra',
      price: 200,
      description: 'A true package for beast mode',
      duration: 30,
      sessions: 10,
      packageType: 'PRO',
      packageImageUri:
        'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NXx8Zml0bmVzc3xlbnwwfHwwfHw%3D&auto=format&fit=crop&w=500&q=60',
      screen: '',
    },
  ];
  
  export const trainers = [
    {
      id: 1,
      name: 'Alex Smith',
      image: 'https://free-images.com/sm/8c15/man_coach_fitness_coach.jpg',
      experience: '2 years experience',
      rating: 4,
      weightType: 'Functional Strength',
      screen: 'Appointment',
    },
    {
      id: 2,
      name: 'Sarah Weghan',
      image:
        'https://media.istockphoto.com/id/1190259052/photo/two-young-women-lifting-weights-in-the-gym.jpg?b=1&s=170667a&w=0&k=20&c=mILsYRe4z5cJ_o9_nPS7O2at1ZLa4CwLkgVn41VVoJ8=',
      experience: '2 years experience',
      rating: 4,
      weightType: 'Heavy Weight Training',
      screen: 'Appointment',
    },
    {
      id: 3,
      name: 'Alex Smith',
      image: 'https://free-images.com/sm/8c15/man_coach_fitness_coach.jpg',
      experience: '2 years experience',
      rating: 4,
      weightType: 'light Weight Training',
      screen: 'Appointment',
    },
    {
      id: 4,
      name: 'Sarah Weghan',
      image:
        'https://media.istockphoto.com/id/1190259052/photo/two-young-women-lifting-weights-in-the-gym.jpg?b=1&s=170667a&w=0&k=20&c=mILsYRe4z5cJ_o9_nPS7O2at1ZLa4CwLkgVn41VVoJ8=',
      experience: '2 years experience',
      rating: 4,
      weightType: 'Heavy Weight Training',
      screen: 'Appointment',
    },
    {
      id: 5,
      name: 'Alex Smith',
      image: 'https://free-images.com/sm/8c15/man_coach_fitness_coach.jpg',
      experience: '2 years experience',
      rating: 4,
      weightType: 'light Weight Training',
      screen: 'Appointment',
    },
    {
      id: 6,
      name: 'Sarah Weghan',
      image:
        'https://media.istockphoto.com/id/1190259052/photo/two-young-women-lifting-weights-in-the-gym.jpg?b=1&s=170667a&w=0&k=20&c=mILsYRe4z5cJ_o9_nPS7O2at1ZLa4CwLkgVn41VVoJ8=',
      experience: '2 years experience',
      rating: 4,
      weightType: 'Heavy Weight Training',
      screen: 'Appointment',
    },
  ];
  

  export const donaters = [
    {
      id: 1,
      name: 'Alex Smith',
      image: 'https://media.istockphoto.com/id/1387498662/photo/unloading-the-truck.jpg?b=1&s=170667a&w=0&k=20&c=Vh_uBwARA57mvPmEIyNwYyiFF5IoMZV5MhBqIVGw3R0=',
      experience: '10 followers',
      rating: 4,
      weightType: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae magnam, doloremque veniam sed animi ab mollitia? Fugit delectus sequi suscipit.',
      screen: '',
    },
    {
      id: 2,
      name: 'Sarah Weghan',
      image:'https://media.istockphoto.com/id/1387498662/photo/unloading-the-truck.jpg?b=1&s=170667a&w=0&k=20&c=Vh_uBwARA57mvPmEIyNwYyiFF5IoMZV5MhBqIVGw3R0=',
        experience: '10 followers',
      rating: 4,
      weightType: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae magnam, doloremque veniam sed animi ab mollitia? Fugit delectus sequi suscipit.',
      screen: '',
    },
    {
      id: 3,
      name: 'Alex Smith',
      image: 'https://media.istockphoto.com/id/1338654343/photo/mother-and-son-preparing-a-box-of-clothes-for-donation.jpg?b=1&s=170667a&w=0&k=20&c=9Le9qPvZNo23qTqyKaAuJDGQlf4f9QGD9P9Mjm5ivjg=',
      experience: '10 followers',
      rating: 4,
      weightType: ' Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae magnam, doloremque veniam sed animi ab mollitia? Fugit delectus sequi suscipit.',
      screen: '',
    },
    {
      id: 4,
      name: 'Sarah Weghan',
      image:'https://media.istockphoto.com/id/1190259052/photo/two-young-women-lifting-weights-in-the-gym.jpg?b=1&s=170667a&w=0&k=20&c=mILsYRe4z5cJ_o9_nPS7O2at1ZLa4CwLkgVn41VVoJ8=',
        experience: '10 followers',
      rating: 4,
      weightType: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae magnam, doloremque veniam sed animi ab mollitia? Fugit delectus sequi suscipit.',
      screen: '',
    },
    
  ];
  